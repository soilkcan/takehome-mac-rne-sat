...
On a tie, "even" refers to `q` — just test its low bit `q[0]`. Do not look
at the rounded result's parity: `q` and `q + 1` are always opposite, so
`q[0]` alone decides the tie.
